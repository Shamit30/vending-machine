#include <thrift/protocol/TBinaryProtocol.h>
#include <thrift/server/TThreadedServer.h>
#include <thrift/transport/TServerSocket.h>
#include <thrift/transport/TBufferTransports.h>
#include <signal.h>

#include "../utils.h"
#include "BeveragePreferenceHandler.h"

using json = nlohmann::json;
using apache::thrift::server::TThreadedServer;
using apache::thrift::transport::TServerSocket;
using apache::thrift::transport::TFramedTransportFactory;
using apache::thrift::protocol::TBinaryProtocolFactory;

using namespace vending_machine;

// signal handler for graceful shutdown
void sigintHandler(int sig) {
    exit(EXIT_SUCCESS);
}

// entry point of beverage-preference-service
int main(int argc, char **argv) {
    // 1: Attach signal handler
    signal(SIGINT, sigintHandler);

    // 1.1: Initialize logger
    init_logger();

    // 2: Read the config file for ports and addresses
    json config_json;
    if (load_config_file("config/service-config.json", &config_json) != 0) {
        exit(EXIT_FAILURE);
    }

    // 3: Fetch port number from config
    int my_port = config_json["beverage-preference-service"]["port"];

    // 4: Configure and start the Thrift server
    TThreadedServer server(
        std::make_shared<BeveragePreferenceServiceProcessor>(
            std::make_shared<BeveragePreferenceServiceHandler>()),
        std::make_shared<TServerSocket>("0.0.0.0", my_port),
        std::make_shared<TFramedTransportFactory>(),
        std::make_shared<TBinaryProtocolFactory>()
    );

    // 5: Run the server
    std::cout << "Starting the beverage preference service ..." << std::endl;
    server.serve();
    return 0;
}
